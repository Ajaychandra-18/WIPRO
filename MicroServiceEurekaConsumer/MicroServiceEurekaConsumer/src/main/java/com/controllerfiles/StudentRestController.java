...
public class StudentRestController {
	...
	return "Accessing from MicroServiceEurekaProducer ==> "+consumer.getBookData();// # 1 INTERFACE METHOD
	 }
	
	@GetMapping("/allBooks")
	public String getBooksInfo() {
	return "Accessing from MicroServiceEurekaProducer ==> " + 	consumer.getAllBooks();// # 2 INTERFACE METHOD
	 }
	@GetMapping("/getOneBook/{id}")
	public String getOneBookForStd(@PathVariable(value="id") Integer id) {
	return "Accessing from MicroServiceEurekaProducer ==> " + 	consumer.getBookById(id); // # 3 INTERFACE METHOD
	 }
	@GetMapping("/entityData")
	public String printEntityData() {
	 ResponseEntity<String> resp = consumer.getEntityData();// # 4 INTERFACE 	METHOD
	return "Accessing from MicroServiceEurekaProducer ==> " + resp.getBody() +" ,	status is:" +	resp.getStatusCode();
	 }
